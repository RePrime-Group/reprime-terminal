  ...
  for (const deal of deals) {
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        coc: metrics.cocReturn !== null ? metrics.cocReturn.toFixed(2) : null,
    ...
    if (updateError) {
      ...
      console.log(`  ✓ ${deal.name} — cap ${metrics.capRate.toFixed(2)}% | irr ${metrics.irr?.toFixed(2) ?? '—'}% | coc ${metrics.cocReturn !== null ? metrics.cocReturn.toFixed(2) + '%' : '∞'} | dscr ${metrics.combinedDSCR.toFixed(2)}`);
      updated++;
    }
  }
