  ...
  return (
    <BaseLayout preview={`Welcome to RePrime Terminal, ${firstName} — your account is active`}>
      ...
      <table cellPadding="0" cellSpacing="0" width="100%" style={{ borderCollapse: 'collapse', margin: '24px 0' }}>
        {[
          { icon: '🏢', title: 'Active Deals', desc: 'Browse current off-market opportunities with full financial metrics' },
          { icon: '📊', title: 'Data Rooms', desc: 'Access due diligence materials — financials, legal, environmental' },
          { icon: '📈', title: 'Model Returns', desc: 'Interactive financial modeling with custom assumptions' },
          { icon: '📅', title: 'Meet the Team', desc: 'Schedule calls directly with our acquisition team' },
        ].map((item, i) => (
          <tr key={i}>
            <td style={{ padding: '12px 0', borderBottom: i < 3 ? '1px solid #EEF0F4' : 'none', verticalAlign: 'top' }}>
              <table cellPadding="0" cellSpacing="0" style={{ borderCollapse: 'collapse' }}>
                <tr>
                  <td style={{ width: '40px', verticalAlign: 'top', paddingRight: '12px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '10px', backgroundColor: '#F7F8FA', textAlign: 'center' as const, lineHeight: '36px', fontSize: '18px' }}>
                      {item.icon}
                    </div>
                  </td>
                  <td>
