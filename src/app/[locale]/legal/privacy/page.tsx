...
import { PRIVACY_TITLE, PRIVACY_BODY } from '@/lib/legal/privacy-text';

export default async function PrivacyPage({ params }: { params: Promise<{ locale: string }> }) {
  await params;
  const blocks = PRIVACY_BODY.split('\n\n');
    ...
    <div className="min-h-screen px-4 py-16" style={{ backgroundColor: '#07090F' }}>
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-8">
        <div className="flex flex-col items-center gap-4">
          ...
          <h1 className="text-2xl font-semibold text-white">{PRIVACY_TITLE}</h1>
        </div>
