...
import { CONTACT_METHODS, LIFECYCLE_MAP } from './CrmConstants';
...
export default function CrmInvestorCard({
  ...
  const lifecycle = LIFECYCLE_MAP[investor.lifecycle_state];
  ...
  return (
    ...
    >
      <div className="p-4 flex flex-col gap-3 flex-1">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {lifecycle && (
              <span
                className={`inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded text-[10px] font-semibold ${lifecycle.pill}`}
                title={`Lifecycle: ${lifecycle.label}`}
              >
      ...
      <div className="flex items-center gap-4 px-4 py-2.5 border-t border-rp-gray-200 bg-rp-gray-100/40 text-[11px] text-rp-gray-500">
        <span>
          {investor.mandate_count} {t('mandatesLabel')}
        </span>
        <span>
