...
export default function MessageBubble({ message, onOpenCitation, stream = false, refCallback, onEdit }: Props) {
  ...
  return (
    ...
    >
      <div className={`flex items-center gap-1.5 max-w-[88%] min-w-0 ${isUser ? 'flex-row-reverse' : ''}`}>
        <div
          className={`min-w-0 max-w-full rounded-2xl px-4 py-2.5 text-[15px] leading-[1.6] break-words ${
            isUser
