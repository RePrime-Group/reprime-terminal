...
import type { DealFilterController, SortKey, SortDir } from './useDealFilters';

/**
 * The shared search / filter / sort bar used by the Dashboard, Marketplace and
 * the curated group tab. Pure presentation — all state lives in the controller
 * returned by useDealFilters(). Includes the -mt-14 hero overlap wrapper so
 * callers can drop it straight under their hero band.
 */
export default function DealFilterBar({ controller }: { controller: DealFilterController }) {
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        {hasActiveFilters && (
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            <span className="text-[10px] text-[#9CA3AF] font-medium">{resultsCount} {t('resultsFound')}</span>
            {searchQuery && (
            ...
            {[...selectedTypes].map((pt) => (
              <span key={pt} className="inline-flex items-center gap-1 px-2 py-0.5 bg-[#0E3470]/8 text-[#0E3470] rounded-md text-[10px] font-semibold">
                {pt}
                <button onClick={() => toggleType(pt)} className="hover:text-[#DC2626] transition-colors"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
              </span>
            ))}
            {sellerFinancingOnly && (
